...
class ComponentContextStripV3 extends HTMLElement{
  ...
  _render(){
    ...
    const action=this._action(),tag=action?'button':'div',attrs=action?' type="button"':'';
...
.strip{appearance:none;width:100%;min-height:44px;box-sizing:border-box;border:0;background:transparent;font:inherit;padding:12px 14px;display:grid;grid-template-columns:minmax(0,1fr) auto minmax(0,1fr);align-items:center;gap:16px;font-size:11.5px;line-height:1.3;white-space:nowrap;overflow:hidden;color:inherit}.strip.actionable{cursor:pointer}
button.strip:active{transform:scale(.997)}button.strip:focus-visible{outline:2px solid var(--primary-color);outline-offset:-2px;border-radius:var(--ha-card-border-radius,16px)}
...
@media(max-width:900px){.strip{gap:10px;padding:11px 12px;font-size:11px}.mid{gap:10px}.item{gap:3px}}
@media(max-width:650px){.strip{font-size:11px;gap:6px;padding:10px}.mid{gap:7px}}
</style><ha-card><${tag} class="strip ${action?'actionable':''}"${attrs}><span class="phase">${escapeHtml(this.c.left_text)}</span><span class="mid">${[1,2,3].map(i=>`<span class="item"><span class="lab">${escapeHtml(this.c[`center_${i}_label`])}</span><span class="val">${escapeHtml(this.c[`center_${i}_value`])}</span></span>`).join('')}</span><span class="event">${escapeHtml(this.c.right_text)}</span></${tag}></ha-card>`;
    if(action)this._interaction=interaction(this.shadowRoot.querySelector('.strip'),{primary:action,optimistic:false,repeat:false,feedback:true});
  }
}
