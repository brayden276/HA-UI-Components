...
const F = { PAUSE:1, MUTE:8, PREVIOUS:16, NEXT:32, PLAY:512, STEP_VOLUME:1024, SOURCE:2048, STOP:4096 };
...
class ComponentAppleTvControllerV1 extends HTMLElement {
  ...
  volumeControl(model){const control=document.createElement("div"),readout=document.createElement("span"),value=document.createElement("span"),status=document.createElement("span");control.className="volume-control";readout.className="volume-readout";value.className="volume-value";status.className="volume-status";value.textContent=model.level===null?"—":Math.round(model.level*100)+"%";status.textContent=model.muted?"Muted":this.busy("volume-down")||this.busy("volume-up")?"Adjusting":"Volume";readout.append(value,status);control.append(this.button("button volume-button","Volume down","mdi:volume-minus",()=>this.adjustVolume("down"),!model.canVolumeDown||this.busy("volume-down"),this.busy("volume-down")),readout,this.button("button volume-button","Volume up","mdi:volume-plus",()=>this.adjustVolume("up"),!model.canVolumeUp||this.busy("volume-up"),this.busy("volume-up")));return control}
  renderQuick(model){const quick=this.el.quick;quick.replaceChildren();if(model.canVolumeDown||model.canVolumeUp)quick.append(this.volumeControl(model));else if(model.canWake||model.canSleep){const wake=model.canWake,action=wake?"wake":"sleep";quick.append(this.button("button",wake?"Wake Apple TV":"Sleep Apple TV",wake?"mdi:power":"mdi:power-sleep",()=>this.remoteCommand(wake?"wakeup":"suspend",action),this.busy(action),this.busy(action)))}}
  ...
  volume(model){if(!model.canVolumeDown&&!model.canVolumeUp&&!model.canMute)return null;const section=this.section("Volume");if(model.canVolumeDown||model.canVolumeUp){const row=document.createElement("div");row.className="volume-modal";row.append(this.volumeControl(model));section.append(row)}if(model.canMute)section.append(this.button("action",model.muted?"Unmute":"Mute",model.muted?"mdi:volume-high":"mdi:volume-mute",()=>this.mute(model),this.busy("mute"),this.busy("mute")));return section}
...
globalThis.__HA_COMPONENT_LIBRARY__ = Object.freeze({ version: "9.0.0", components: 38 });
